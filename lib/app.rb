...
def dire_bonjour(login_twitter)
...
#selectioner 5 éléments aléatoirement dans le tableau
people_tweet = arr.sample(5)

tell_messages = ["Quel est l'accessoire préféré des chiens guitaristes ? On est dans de beaux draps !","Quelle est la différence entre un pigeon ? Un lapin aquatique.
","Que peut-on faire avec un vélo des années 50 ? Parce qu'ils ont Quimper !","Pourquoi la poule a-t-elle traversé la route ? Un chou-marin ruche !","Qu'est-ce qui est rouge et qui commute ? Un concombre imitant Super-Tomate.","Comment font les chasseurs pour attirer les lapins ? Jonathan !","Comment appelle-t-on une pyramide avec un masque ? Bah t'en pousses un !","Dans quel pays habitent les crapauds ? La Roche Posay !","Pourquoi Perrache ne tient pas debout ? Il s'aperçoit qu'il a pas de pattes, et alors il tombe.","Monsieur et Madame Talu ont 4 fils, comment s'appellent-ils ? L'iPhone marche !","Quel est le point commun entre un facteur et un jongleur ? La Drôme, car la Drôme adhère !","Comment appelle-t-on une association qui nourrit les personnes dans le besoin avec de la sauce au basilic? Une poule de cristal !"]

people_tweet.map{|p| login_twitter.update ("#bonjour_monde Salut #{p} tu veux une petite blague qui fait rire: #{tell_messages.sample(1).join(' ')} Merci à @the_hacking_pro la meilleure formation développeur web")}
...
#dire_bonjour(login_twitter)

def liker_bonjour(login_twitter)
...
login_twitter.favorite(tweets)
#login_twitter.follow(tweets)

#liker les tags récupérés
...
#liker_bonjour(login_twitter)

#follow_bonjour(login_twitter)
